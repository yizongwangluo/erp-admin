...
function uploadAllComplete()//全部文件上传成功
{
    ...
    {
        var curpicobj=$(window.parent.document).find("#h_pic").val();
        $(window.parent.document).find('#'+curpicobj).val(arrMsg[0].url);
        //$(window.parent.document).find("#img_pic").attr('src',arrMsg[0].url);
