            ...
            if($operate){
                ...
                if(empty($operate['exchange_rate']) || empty($operate['service_charge'])){
                    ...
                    if(empty($operate['ad_cost'])){//广告费未上传时,数据为空
                        ...
                        //ROI=营业额/广告费
                        $ROI = max(0,bcdiv($data['turnover'],$operate['ad_cost'],2));

                        //每单广告成本=广告费/付款订单数
                        $unit_ad_cost = max(0,bcdiv($operate['ad_cost'],$data['paid_orders'],2));

                        //产品总成本转换为美元
                        $product_total_cost_usd = max(0,bcdiv($data['sku_total_cost'],$operate['exchange_rate'],2));
                        ...
                        //毛利率=毛利/营业额
                        $gross_profit_rate = max(0,bcdiv($gross_profit,$data['turnover'],9));
