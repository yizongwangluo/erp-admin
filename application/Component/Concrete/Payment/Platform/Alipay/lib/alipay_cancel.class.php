...
class AliCancel{
	...
	protected function dejson($url){
		...
		if ($d['alipay_trade_close_response']['code'] =='10000'){
			$flag = $this->getRsaVerify ($d['alipay_trade_close_response'],$d['sign']);
			if (!$flag){
				log_message ('alipay_refund_error_logs','\'支付签名校验失败！\'',true);
				return false;
			}
			return $d['alipay_trade_close_response']['trade_no'];
		}else{
			if ($retry < 3){
				$retry++;
				$this->dejson ($url);
